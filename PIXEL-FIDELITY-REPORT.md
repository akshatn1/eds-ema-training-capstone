...
> ## Pass 4 — Corrections after failed validation (branch `feat/dynamic-listings`, from main `97c00c0`)
>
> **Corrections to earlier inaccurate claims:**
> - PR #9 was **CLOSED AND MERGED** (not unmerged as previously stated). main = `97c00c0`.
> - The dynamic-listing commits landed on `fix/wknd-layout-fix` **after** PR #9 merged, so they were never in main; PR #9 did not deliver them.
> - The prior `cards-dynamic` block was NOT visually correct: adding `class="cards"` at runtime does not make EDS load `cards.css`, so Magazine rendered as full-width images with blue serif titles.
> - Static rows were NOT fully removed: Adventures kept the old `<ol>` filter list + static `<ul>` card groups under the dynamic list (duplication). Home ordering fell back to alphabetical (blank dates).
> - The query index was a hand-published static snapshot with blank dates — not auto-updating.
> - Publishing dynamic blocks to the public DA docs while the code was only on a branch left **main exposing raw config rows** (production regression).
>
> ### P0 — production regression fixed
> Restored the three public DA documents (Home/index, Magazine, Adventures) to their last-known-good **static** content and republished to live. Verified on main live: `cards-dynamic`=0, raw config rows=0, static cards restored (home 2, magazine 1, adventures 1). **main is no longer broken.**
>
> ### Dynamic listing rebuilt as the proper EDS variant
> - Block authored as **Cards (dynamic)** → classes `cards dynamic` → EDS loads `blocks/cards/cards.js` + `blocks/cards/cards.css`. `cards.js` detects the `dynamic` class and runs the index renderer; **shared fetch/filter/sort lives in `scripts/listing.js`** (imported by cards.js — no cross-block import). `blocks/cards-dynamic` removed. The WKND `.cards` CSS now applies → correct four-column borderless cards.
> - Header un-stuck: `styles.css` `position: sticky` → `relative`.
>
> ### Data contract (query-index.json columns)
> `path, title, description, image, lastModified, publicationDate, template, type, category, tags, membersOnly, homeFeatured, homeOrder`. Derived from each page's metadata + the WKND source page's own filter-tab / home-section membership.
>
> ### Authored config cells
> `path, type, category, members, exclude, featured, sort, direction, limit, filters` (=`category` → keyboard-accessible tabs).
>
> ### Listing rules (verified by running the real cards.js against the live index)
> | Listing | Config | Result |
> |---|---|---|
> | Home Recent Articles | type=article, featured=true, members=false, sort=homeOrder asc, limit=4 | **LA Skateparks, Ski Touring, Arctic Surfing, San Diego** — exact WKND order (no alphabetical fallback) |
> | Home Where-to-go | type=adventure, featured=true, sort=homeOrder asc, limit=4 | **Yosemite, Whistler, West Coast, Tahoe** — exact WKND selection |
> | Magazine All Articles | type=article, members=false | all **5** public articles once |
> | Adventures | type=adventure, filters=category | all **16** once; tabs All/Climbing/Cycling/Skiing/Surfing/Travel; Surfing→2 |
> | (impossible filter) | — | `.cards-empty` empty state |
>
> ### Validation
> - `/query-index.json` → **200** on preview + live (26 rows, `:type: sheet`, now with publicationDate + homeFeatured + homeOrder).
> - Preview-only test doc `/drafts/listing-test` (noindex; public pages left static): mobile **Lighthouse Perf 99 / A11y 100**, CLS 0.077, LCP 0.9s; real-Chromium audits `image-alt`/`link-name`/`heading-order`/`list` all **PASS** (cards rendered as accessible ul>li with images+titles).
> - All **21** indexed listing images resolve **200** (no broken images).
> - `npm run lint` clean. `scripts/aem.js` untouched.
>
> ### Honest remaining blockers / differences
> - **tools.aem.live index is an unresolved blocker.** I cannot reach the project config UI/API, so the index is still a DA-published sheet generated by `tools/importer/build-query-index.mjs` — it will NOT auto-update on new publishes. **Required action for the user:** in tools.aem.live, define an index for this site exposing columns `path, title, description, image, lastModified, publicationDate, template/type, category, tags, membersOnly, homeFeatured, homeOrder` (mapped from page metadata), so `/query-index.json` regenerates on publish. Until then the sheet must be regenerated with the script.
> - **Real-browser interaction (Playwright) was unavailable this session** (sandbox disconnected); rendering was validated via real-Chromium Lighthouse + the actual cards.js executed against the live index in jsdom. Full pointer/keyboard-in-browser re-check is pending browser availability.
> - **Public DA pages remain static** by design (safe sequence): convert them to the `cards dynamic` blocks only after this code PR is reviewed and merged. Draft configs are staged in `migration-work/dyn/`.
> - Members Only (magazine): those cards link to `/magazine/members-only/*` pages that are **out of the migrated /us/en scope** and not in the index, so that source section remains authored (documented, not claimed dynamic).
>
> ---
